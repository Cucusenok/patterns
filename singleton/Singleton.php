<?php

namespace Singleton;


class Singleton
{

    private static $instances = array();

    /**
     * Определим конструктор базового
     * Singleton класса и делаем его protected,
     * чтобы мы погли создавать дочерние классы
     * для классов от которых наследование не планируется
     * можно использовать final class и сделать
     * конструктор private
     * Singleton constructor.
     */
    protected function __construct(){}


    /**
     * Клонирование и десериализация не разрешены для singleton.
     */
    protected function __clone() {}


    public function __wakeup(){
        throw new \Exception("Cannot unserialize singleton");
    }


    /**
     * @return static
     */
    public static function getInstance()
    {
        $subclass = static::class;
        if (!isset(self::$instances[$subclass])) {
            // Обратите внимание, что здесь мы используем ключевое слово
            // "static"  вместо фактического имени класса. В этом контексте
            // ключевое слово "static" означает «имя текущего класса». Эта
            // особенность важна, потому что, когда метод вызывается в
            // подклассе, мы хотим, чтобы экземпляр этого подкласса был создан
            // здесь.

            self::$instances[$subclass] = new static;
        }
        return self::$instances[$subclass];
    }
}